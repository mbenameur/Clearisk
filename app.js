...
CK.statusDot   = {'Ouvert':'#E24B4A','En cours':'#EF9F27','Atténué':'#1D9E75','Traité':'#94A3B8','À traiter':'#EF9F27','Surveillé':'#94A3B8'};  /* rétrocompat anciens statuts */
...
function showPage(page,el){
  ...
  if(page==='overview' && _dataReady)renderDashboard(); /* évite un rendu prématuré avant chargement Supabase */
...
var rtStatutOptions = [
  ...
  {val:'Ouvert', label:'Ouvert', dot:''},
  {val:'En cours', label:'En cours', dot:''},
  {val:'Atténué', label:'Atténué', dot:''},
...
function renderCartoTable(){
  ...
  sorted.forEach(function(r){
    ...
    var responsableHTML = initiales
      ?'<span class="rt-resp-val">'+initiales+'</span>'
      :'<span class="rt-resp-empty">—</span>';
    // Colonnes : Risque | Catégorie | Niveau | Responsable | Statut
...
function toggleCartoDropdown(type, thEl){
  ...
  if(type==='cat'){
    ...
    options=[
      {val:'tous',label:'Tous les statuts',dot:''},
      {val:'Ouvert',label:'Ouvert',dot:'#EF9F27'},
      {val:'En cours',label:'En cours',dot:'#1A5CAB'},
      {val:'Atténué',label:'Atténué',dot:'#8B5CF6'},
      {val:'Surveillé',label:'Surveillé',dot:'#94A3B8'},
      {val:'Traité',label:'Traité',dot:'#1D9E75'},
      {val:'À traiter',label:'À traiter (ancien)',dot:'#EF9F27'}
    ];
...
function calcScoreDynamic() {
  ...
  riskData.forEach(function(r) {
    if(r.status === 'Traité' || r.status === 'Atténué') return; /* Atténué = risque sous contrôle, pas de pénalité pleine */
...
function _renderDashboardReal() {
  ...
  var _u = riskData.filter(function(r){ return r.level==='critique' && r.status!=='Traité' && r.status!=='Atténué'; }).length;
  ...
  var top5 = sorted.filter(function(r){ return r.status!=='Traité' && r.status!=='Atténué'; }).slice(0,5);
...
var _dataReady = false; /* true uniquement après que sbLoadAllData() a terminé */
...
function generateNotifs() {
  ...
  riskData.forEach(function(r) {
    if(r.level === 'critique' && r.status !== 'Traité' && r.status !== 'Atténué') {
      notifs.push({
        id: 'risk_' + r.id,
        type: 'critique',
        color: '#E24B4A',
...
(function() {
  ...
  // Suppression principale : à la fin de l'animation CSS
  overlay.addEventListener('animationend', function(e) {
    if (e.animationName === 'wFadeOut') removeOverlay();
  });

  // Fallback de sécurité : supprime l'overlay au bout de 4 secondes
  // même si l'événement animationend n'a pas été reçu (bug navigateur, perf lente)
