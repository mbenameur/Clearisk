...
CK.statusDot   = {'Ouvert':'#E24B4A','En cours':'#EF9F27','Atténué':'#1D9E75','Traité':'#94A3B8','À traiter':'#EF9F27','Surveillé':'#94A3B8'};
...
function renderCartoTable(){
  ...
  sorted.forEach(function(r){
    ...
    tr.innerHTML = '<td style="color:#71869A;">'+escHtml(r.cat)+'</td>'
      ...
      + '<td>'+responsableHTML+'</td>'
      + '<td><span style="display:inline-block;font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;background:'+dot+'22;color:'+dot+';border:1px solid '+dot+'55;white-space:nowrap;">'+r.status+'</span></td>';
...
function openAddRisk(){
  ...
  document.getElementById('f-status').value='Ouvert';
...
function saveNewRisk(){
  ...
  var r={
    ...
    status:document.getElementById('f-status').value||'Ouvert',
...
function applyBiblioPanel(){
  ...
  bibCats.forEach(function(cat){
    cat.risks.forEach(function(r){
      ...
      var newRisk={id:r.id,name:r.name,cat:cat.name,level:lvl,status:'Ouvert',date:"À l'instant",source:'lib',color:colMap[lvl],impact:2,proba:1};
...
function buildOnboardingRisks(sector, problemes) {
  ...
  return ordered.map(function(r) {
    return {
      ...
      status:'Ouvert', date:"Aujourd'hui", source:'lib',
...
var DIAG_RISK_ENGINE=[
  ...
  {id:'r_pret',n:'Défaillance — remboursement prêt',cat:'Financier',level:function(a){return 'critique';},cond:function(a){return a.pret===0;},impact:3,proba:function(a){return 2;},status:'Ouvert',kri:'Tréso < prochain remboursement',desc:'Échéances de prêt non couvertes par la trésorerie disponible. Risque bancaire immédiat.',actions:['Contacter sa banque pour renégocier l\'échéancier','Demander un différé partiel de remboursement']},
  ...
  {id:'r_controle',n:'Risque de redressement / contrôle',cat:'Juridique',level:function(a){return a.controle===0?'critique':'modere';},cond:function(a){return a.controle===0;},impact:3,proba:function(a){return 2;},status:'Ouvert',kri:'Redressement en cours',desc:'Un redressement passé augmente significativement le risque de nouveau contrôle.',actions:['Mettre en conformité les points signalés lors du contrôle','Faire un audit préventif par un expert-comptable','Documenter toutes les pratiques commerciales et RH']},
  {id:'r_assurance',n:'Couverture assurantielle insuffisante',cat:'Juridique',level:function(a){return a.assurance===0?'critique':'eleve';},cond:function(a){return a.assurance<=1;},impact:3,proba:function(a){return 2;},status:'Ouvert',kri:'Pas de RC Pro ou perte d\'exploitation',desc:'Sans RC Pro ou assurance perte d\'exploitation, un sinistre peut mettre fin à l\'activité.',actions:['Souscrire une RC Pro adaptée à l\'activité','Ajouter une garantie perte d\'exploitation','Faire comparer les offres par un courtier']},
];
    ...
    if(resRisks.data && resRisks.data.length > 0) {
      ...
      riskData = resRisks.data.map(function(r) {
        return {
          ...
          status: (r.status==='À traiter'?'Ouvert':r.status)||'Ouvert', date: r.created_at ? r.created_at.slice(0,10) : '',
